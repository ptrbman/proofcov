...
"""
    ...
    that can later be included/excluded from
    check-sat-assume().
"""
def add_assumption_literals(smt_text: str) -> str:
    lines = smt_text.strip().splitlines()
    new_lines = []
    assumption_literals = []
    all_literals = []
    ...
    for line in lines:
        ...
        if m:
            ...
            # Always declare and use them in assertions
            all_literals.append(a_name)
    ...
    if assumption_literals:
        joined = " ".join(assumption_literals)
        new_lines.append(f"(check-sat-assuming ({joined}))")
    
    return "\n".join(new_lines)

# Takes an unsat core and converts it to line numbers
...
formula = add_assumption_literals(formula)

if args.verbose:
    print(Panel.fit("[purple]SMT formula:[/purple]"))
    print(formula)

muses = get_MUSes(formula)
cores = []

# If MUSes is empty the problem is SAT
if not muses:
    print("Test [red]fail")
    sys.exit(1)

for mus in muses:
    line = core_to_lines(mus)
    cores.append(line)

# Enumerate all unsat cores
for id, core in enumerate(cores, start=1):
    # 5. If BMC returns a core, extract the lines and subexpressions
    result = core
    if args.verbose:
        print(Panel.fit(f"[blue]Unsat core {id}:[/blue]"))
        print(f"{result}")
    ...
    if args.line:
        ...
        if args.graph:
            cfg_image_path = filename.rsplit('.', 1)[0] + f"_line_core{id}.png"
